...
import 'package:flutter_open_pager/rx_widgets/widget_selector.dart';
...
class RxSpinner extends StatelessWidget {
  ...
  const RxSpinner(
      {this.busyEvents, this.platform, this.radius = 20.0, this.backgroundColor, this.value, this.valueColor, this.strokeWidth: 4.0, this.normal, Key key})
  ...
  Widget build(BuildContext context) {
    ...
    return new WidgetSelector(
      ...
      onTrue: new Center(child: new Container(width: this.radius * 2, height: this.radius * 2, child: spinner)),
...
class _RxLoaderState<T> extends State<RxLoader<T>> {
  ...
  Widget build(BuildContext context) {
    ...
    if (lastReceivedItem.isExecuting) {
      return new Center(child: new Container(width: this.widget.radius * 2, height: this.widget.radius * 2, child: spinner));
    }
